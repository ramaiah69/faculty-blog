...
<?php
		...
		echo '<center>';
		echo '<span class="titl">Mtech Panel</span>';
		echo '<table >';
		...
		$id=1;
		$query=mysql_query("select * from  mtech");
			while($data=mysql_fetch_assoc($query))
			{
					...
					else
					{
						...
						echo '<td><a href="student_edit.php?id='.$data['id'].'&type=mtech&year='.$data['year'].'" target="_blank">Edit</a></td>';
						echo '<td id="del">';
						echo '<a href="#" onClick="del2('."'".$data['id']."','mtech',".$data['year'].')" >Delete</a></td></tr>';
